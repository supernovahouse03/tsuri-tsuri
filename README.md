...
## さかなの え を つくりなおす
`src/assets/fish/<さかなID>.webp` が あれば その えを、なければ `art.jsx` の SVG を つかう。

```
npm run gen:fish              # まだ 無い さかな だけ 生成
npm run gen:fish -- saba fugu # この さかな だけ 作りなおし
npm run gen:fish -- --all     # ぜんぶ 作りなおし
```
